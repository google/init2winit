...
class TrainingMetricsGrabberTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    self.mock_params0 = freeze(
        {'foo': jnp.zeros(5), 'bar': {'baz': jnp.ones(10)}}
    )
    self.mock_batch_stats = freeze(
        {'foo': jnp.zeros(5), 'bar': {'baz': jnp.ones(10)}}
    )
    self.mock_grad1 = freeze(
        {'foo': -jnp.ones(5), 'bar': {'baz': -jnp.ones(10)}}
    )
    self.mock_grad2 = freeze(
        {'foo': -2 * jnp.ones(5), 'bar': {'baz': -2 * jnp.ones(10)}}
    )

    self.mock_nu0 = freeze(
        {'foo': 2 * jnp.ones(5), 'bar': {'baz': 3 * jnp.ones(10)}}
    )
    self.mock_nu1 = freeze(
        {'foo': 3 * jnp.ones(5), 'bar': {'baz': 4 * jnp.ones(10)}}
    )

    self.mock_optimizer_state0 = InjectHyperparamsState(
        0, None, (ScaleByAdamState(0, None, self.mock_nu0),)
    )

    self.mock_optimizer_state1 = InjectHyperparamsState(
        0, None, (ScaleByAdamState(1, None, self.mock_nu1),)
    )
    ...
    self.mock_params1 = jax.tree.map(
        lambda p, g: p - self.step_size * g, self.mock_params0, self.mock_grad1
    )
    self.mock_params2 = jax.tree.map(
        lambda p, g: p - self.step_size * g, self.mock_params1, self.mock_grad2
    )

    self.mock_zeros = freeze(
        {'foo': jnp.zeros(5), 'bar': {'baz': jnp.zeros(10)}}
    )
  ...
  def test_init(self):
    ...
    init_fn, _, _ = make_training_metrics(self.num_train_steps, ConfigDict({}))
    initial_metrics_state = init_fn(self.mock_params0, self.mock_batch_stats)
    self.assertTrue(
        pytree_equal({'param_norm': zeros_timeseries}, initial_metrics_state)
    )

    # Test init with enable_ema = True and enable_train_cost=True.
    init_fn, _, _ = make_training_metrics(
  ...
  def test_update_param_norm(self):
    ...
    expected_param_norm = expected_param_norm.at[0].set(
        total_tree_norm_l2(self.mock_params0)
    )
    expected_param_norm1 = expected_param_norm1.at[0].set(
        total_tree_norm_l2(self.mock_params0)
    )
    expected_param_norm1 = expected_param_norm1.at[1].set(
        total_tree_norm_l2(self.mock_params1)
    )
  ...
  def test_optstate_sumsq(self):
    ...
    self.assertEqual(
        updated_metrics_state['optstate_sumsq']['nu'][0],
        total_tree_norm_sql2(self.mock_nu0),
    )
    self.assertEqual(
        updated_metrics_state['optstate_sumsq']['nu'][1],
        total_tree_norm_sql2(self.mock_nu1),
    )

    self.assertEqual(
        updated_metrics_state['optstate_sum']['nu'][0],
        total_tree_sum(self.mock_nu0),
    )
    self.assertEqual(
        updated_metrics_state['optstate_sum']['nu'][1],
  ...
  def test_update_precondition(self):
    ...
    init_fn, update_fn, _ = make_training_metrics(
        self.num_train_steps,
        ConfigDict({'optimizer': optimizer, 'opt_hparams': opt_hparams}),
    ...
    pre0 = make_diag_preconditioner(
        optimizer, opt_hparams, self.mock_optimizer_state0, ConfigDict({})
    )
    semip_grad_0 = jax.tree.map(
        lambda g, p: g / (p**0.5), self.mock_grad1, pre0
    )

    pre1 = make_diag_preconditioner(
        optimizer, opt_hparams, self.mock_optimizer_state1, ConfigDict({})
    )
    semip_grad_1 = jax.tree.map(
        lambda g, p: g / (p**0.5), self.mock_grad2, pre1
    )

    self.assertEqual(
        updated_metrics_state['preconditioner_normsq'][0],
        total_tree_norm_sql2(pre0),
    )
    self.assertEqual(
        updated_metrics_state['preconditioner_normsq'][1],
        total_tree_norm_sql2(pre1),
    )

    self.assertEqual(
        updated_metrics_state['semip_grad_normsq'][0],
        total_tree_norm_sql2(semip_grad_0),
    )
    self.assertEqual(
        updated_metrics_state['semip_grad_normsq'][1],
        total_tree_norm_sql2(semip_grad_1),
    )

  def test_summarize(self):
    """Test the training metrics summarizer."""
    _, _, summarize_fn = make_training_metrics(
        self.num_train_steps,
        ConfigDict({}),
        enable_train_cost=True,
        enable_ema=True,
    ...
    metrics_state = {
        ...
        'param_norm': {'foo': 7.0, 'bar': {'baz': 2.0}},
        'grad_ema': {'foo': 1 * jnp.ones(5), 'bar': {'baz': 2 * jnp.ones(10)}},
        'grad_sq_ema': {
            'foo': 2 * jnp.ones(5),
