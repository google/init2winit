...
class KitchenSinkFailTest(absltest.TestCase):
  ...
  def test_bad_hp(self):
    self.assertRaises(
        TypeError,
        kitchen_sink,
        {'0': {'element': 'nesterov', 'hps': {'asdf': 1e-4}}},
    )

  def test_bad_transform_name(self):
    self.assertRaises(ValueError, kitchen_sink, {'0': {'element': 'rasputin'}})
...
class FromHParamsTest(chex.TestCase):
  ...
  def test_empty_element(self):
    self.assertRaises(
        ValueError, kitchen_sink, ml_collections.ConfigDict({'0': {}})
    )

  def test_empty_hps(self):
    kitchen_sink(ml_collections.ConfigDict({'0': {'element': 'nesterov'}}))

  def test_equal_structs(self):
    """Test that initial and updated states have the same tree structure."""
    tx = kitchen_sink(ml_collections.ConfigDict({'0': {'element': 'nesterov'}}))
    params = {'a': 1.0}
    gradients = {'a': 2.0}
  ...
  def test_empty_mask(self):
    kitchen_sink(
        ml_collections.ConfigDict(
            {'0': {'element': 'nesterov', 'hps': {'decay': 0.9}}}
        )
    )
  ...
  def test_one_minus(self):
    ...
    tx_one_minus = kitchen_sink(
        ml_collections.ConfigDict(
            {'0': {'element': 'nesterov', 'hps': {'one_minus_decay': 0.1}}}
        )
    )
...
class GraftCombinatorTest(chex.TestCase):
    ...
    g = kitchen_sink(
        {
            'join': {
                'mag_chain': {
                    'element': 'sgd',
                    'hps': {'learning_rate': -1.0},
                },
                'dir_chain': {
                    'element': 'sgd',
                    'hps': {'learning_rate': -2.0},
                },
            },
            'by': 'grafting',
        },
  ...
  def test_nn(self):
    ...
    m = optax.sgd(learning_rate=-1.0, momentum=0.9)
    d = optax.sgd(learning_rate=-2.0, momentum=0.9)
    g = kitchen_sink(
        {
            'join': {
                'mag_chain': {
                    'element': 'sgd',
                    'hps': {'learning_rate': -1.0, 'momentum': 0.9},
                },
                'dir_chain': {
                    'element': 'sgd',
                    'hps': {'learning_rate': -2.0, 'momentum': 0.9},
                },
            },
            'by': 'grafting',
        },
