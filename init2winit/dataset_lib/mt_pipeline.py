...
def get_user_defined_symbols(
    ds_info: tfds.core.DatasetInfo, reverse_translation: bool
):
...
def get_sampled_dataset(
  ...
  if loss_weights is not None:
    raw_data = [
        ds.map(ImportanceSamplingOp(weight), num_parallel_calls=AUTOTUNE)
        for ds, weight in zip(raw_data, loss_weights)
    ]

  def _shuffle_repeat(dataset, shuffle_seed: int, shuffle_buffer_size):
  ...
  if pack_examples:
    ...
    if 'weights' in dataset.element_spec:
      dataset = dataset.padded_batch(
          ...
          padding_values={'inputs': 0, 'targets': 0, 'weights': 0.0},
...
def get_wmt_datasets(
  ...
  if per_host_eval_batch_size % n_devices:
    raise ValueError(
        "Eval Batch size %d isn't divided evenly by n_devices %d"
  ...
  train_ds_builders = [
      tfds.builder(tfds_dataset_key) for tfds_dataset_key in dataset_keys
  ]
  ...
  sampled_train_data = get_sampled_dataset(
      ...
      config.train_split,
      dataset_rates,
      config.reverse_translation,
      config.add_language_token,
