...
def split_sequences(sequence, sequence_length):
  """Splits a sequence of arbitrary length into subsequences of length sequence_length."""
  sequence = tf.data.Dataset.from_tensor_slices(
      sequence['input_ids']
  )  # -> Dataset of tokens
  sub_sequences = sequence.batch(sequence_length)  # -> Dataset of subsequences
...
def get_fineweb_edu_dataset(
  ...
  train_sequences_dataset = train_tokens.batch(
      hps.sequence_length, drop_remainder=True
  )
  eval_train_sequences_dataset = train_tokens.batch(
      hps.eval_sequence_length, drop_remainder=True
  )
  ...
  train_sequences_dataset = train_sequences_dataset.map(
      lambda x: {'inputs': x['input_ids'], 'targets': x['input_ids']},
