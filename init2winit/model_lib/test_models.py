...
class ModelsTest(parameterized.TestCase):
  ...
  def test_local_attention_transformer(self):
    ...
    model_hps = models.get_model_hparams(model_str)
    model_hps.update({'output_shape': (98305,), 'batch_size': 8})
    model_cls = models.get_model(model_str)
    dropout_rng, params_rng = jax.random.split(jax.random.PRNGKey(0))
    loss = 'cross_entropy'
    metrics = 'classification_metrics'
    model = model_cls(model_hps, {}, loss, metrics)
    inputs = jnp.array(np.random.randint(size=(1, 8192), low=1, high=98305))
    model_init_fn = jax.jit(
        functools.partial(model.flax_module.init, train=False))
    ...
    outputs, _ = model.flax_module.apply(
        ...
        rngs={'dropout': dropout_rng},
        train=True)
    self.assertEqual(outputs.shape, (1, 8192, 98305))


class LocalAttentionTransformerSupportTests(parameterized.TestCase):
  ...
  def test_shape_list(self):
    """Tests support function shape_list."""
    x = np.array(np.random.rand(1, 2), dtype=np.float32)
    output = local_attention_transformer.shape_list(input_array=x)
  ...
  def test_decode_step_to_index(self):
    ...
    output = local_attention_transformer.decode_step_to_index(
        decode_step=decode_step, array_shape=array_shape)
  ...
  def test_get_item_at_decode_step(self):
    ...
    decode_step = 10
    output = local_attention_transformer.get_item_at_decode_step(
        input_array=x, decode_step=decode_step)
  ...
  def test_embedding_to_padding(self):
    ...
    output = local_attention_transformer.embedding_to_padding(embedding=x)
  ...
  def test_attention_bias_local(self):
    ...
    output = local_attention_transformer.attention_bias_local(
        length=length, max_backward=max_backward, max_forward=max_forward)
  ...
  def test_attention_bias_lower_triangle(self):
    ...
    length = 2
    output = local_attention_transformer.attention_bias_lower_triangle(
        length=length)
  ...
  def test_maybe_tile(self):
    ...
    output = local_attention_transformer.maybe_tile(input_x=x, input_y=y)
  ...
  def test_local_attention_bias_nd(self):
    ...
    x = np.array(np.random.rand(8, 1, 512, 1024), dtype=np.float32)
    output = local_attention_transformer.local_attention_bias_nd(v_array=x)
  ...
  def test_pad_to_multiple_nd(self):
    ...
    block_shape = (2, 10)
    output = local_attention_transformer.pad_to_multiple_nd(
        input_x=x, block_shape=block_shape)
  ...
  def test_select_block_for_decode_step(self):
    ...
    output = local_attention_transformer.select_block_for_decode_step(
        input_x=x, decode_step=decode_step)
  ...
  def test_break_into_blocks_nd(self):
    ...
    block_shape = (3,)
    output = local_attention_transformer.break_into_blocks_nd(
        input_x=x, block_shape=block_shape)
  ...
  def test_break_into_memory_blocks_nd(self):
    ...
    output = local_attention_transformer.break_into_memory_blocks_nd(input_x=x)
  ...
  def test_flatten_blocks_nd(self):
    ...
    output = local_attention_transformer.flatten_blocks_nd(input_x=x)
  ...
  def test_unflatten_blocks_nd(self):
    ...
    output = local_attention_transformer.unflatten_blocks_nd(
        input_x=x, blocks_per_dimension=blocks_per_dimension)
  ...
  def test_break_bias_into_blocks(self):
    ...
    output = local_attention_transformer.break_bias_into_blocks(input_bias=x)
  ...
  def test_cast_like(self):
    ...
    output = local_attention_transformer.cast_like(input_x=x, input_y=y)
  ...
  def test_generate_relative_positions_embeddings(self):
    ...
    output = local_attention_transformer.generate_relative_positions_embeddings(
        length_q=length_q, length_k=length_k, depth=depth, rng_key=rng_key)

    self.assertEqual(output.shape, (10, 10, 5))
    self.assertEqual(output.dtype, np.float32)
  ...
  def test_relative_attention_inner(self):
    ...
    output = local_attention_transformer.relative_attention_inner(
        input_x=x, input_y=y, input_z=z, transpose=transpose)
  ...
  def test_dot_product_attention_relative(self):
    ...
    rng_key = jax.random.PRNGKey(0)
    output = local_attention_transformer.dot_product_attention_relative(
        q_array=q, k_array=k, v_array=v, bias_array=b, rng_key=rng_key)
  ...
  def test_dot_product_attention(self):
    ...
    params = model.init({'params': key1, 'initializer': key2}, q, k, v, b)
    output = model.apply(params, q, k, v, b, rngs={'initializer': key2})
  ...
  def test_combine_last_two_dimensions(self):
    """Tests support function combine_last_two_dimensions."""
    x = np.array(np.random.rand(1, 2), dtype=np.float32)
    output = local_attention_transformer.combine_last_two_dimensions(input_x=x)
  ...
  def test_combine_heads_nd(self):
    ...
    output = local_attention_transformer.combine_heads_nd(input_x=x)
  ...
  def test_put_back_blocks_nd(self):
    ...
    block_shape = (3,)
    output = local_attention_transformer.put_back_blocks_nd(
        input_x=x, block_shape=block_shape)
  ...
  def test_attention_nd(self):
    ...
    params = model.init({'params': key1, 'initializer': key2}, q, k, v)
    output = model.apply(params, q, k, v, rngs={'initializer': key2})

    self.assertEqual(output.shape, (1, 8, 256, 1024))
    self.assertEqual(output.dtype, np.float32)

  def test_split_last_dimension(self):
    """Tests support function split_last_dimension."""
    x = np.array(np.random.rand(10,), dtype=np.float32)
    dim_no = 2
    output = local_attention_transformer.split_last_dimension(
        input_x=x, dim_no=dim_no)
  ...
  def test_split_heads_nd(self):
    ...
    output = local_attention_transformer.split_heads_nd(input_x=x)

    self.assertEqual(output.shape, (8, 8, 1))
    self.assertEqual(output.dtype, np.float32)

  def test_inplace_function(self):
    """Tests support function inplace_function."""
    x = jnp.array(np.random.rand(1, 3, 8, 4), dtype=jnp.float32)
    index = 0
    output = local_attention_transformer.inplace_function(
        input_x=x, index=index)

    self.assertEqual(output.shape, (1, 3, 8, 4))
    self.assertEqual(output.dtype, np.float32)

  def test_put_item_in_decode_step(self):
    """Tests support function put_item_in_decode_step."""
    x = np.array(np.random.rand(3, 8, 1, 4), dtype=np.float32)
  ...
  def test_multihead_attention_nd(self):
    ...
    params = model.init({'params': key1, 'initializer': key2}, x)
    output = model.apply(params, x, rngs={'initializer': key2})

    self.assertEqual(output.shape, (1, 2, 1032))
    self.assertEqual(output.dtype, np.float32)

  def test_get_embeddings(self):
    """Tests support function get_embeddings."""
    x = np.array(np.random.rand(1, 2, 3), dtype=np.int32)
    rng_key = jax.random.PRNGKey(0)
    output = local_attention_transformer.get_embeddings(
        targets=x, rng_key=rng_key)

    self.assertEqual(output.shape, (1, 2, 3, 1032))
    self.assertEqual(output.dtype, np.float32)
  ...
  def test_get_timing_signal_1d(self):
    ...
    length = 10
    channels = 2
    output = local_attention_transformer.get_timing_signal_1d(
  ...
  def test_layer_postprocess(self):
    ...
    params = model.init({'params': key1, 'dropout': key2}, x, y)
    output = model.apply(params, input_x=x, input_y=y, rngs={'dropout': key2})
  ...
  def test_decoder_block(self):
    ...
    params = model.init({'params': key1, 'initializer': key2}, x)
    imp_output = model.apply(params, x, rngs={'initializer': key2})
