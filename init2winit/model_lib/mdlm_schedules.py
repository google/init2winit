...
def get_schedule(name):
  """Returns (alpha_fn, alpha_derivative_fn) for the given noise schedule.

  Each schedule defines alpha_t as a function of t in [0, 1], where
  alpha_0 ~ 1 (clean data) and alpha_1 ~ 0 (fully masked). The derivative
  is used for the continuous-time ELBO loss. The noise schedule defines how
  quickly the noise diffuses into the data. For more details, see
  Sahoo et al. (2024) https://arxiv.org/abs/2406.07524.

  Available schedules:
    - 'log_linear':  alpha_t = 1 - t
    - 'cosine':      alpha_t = cos(pi * t / 2)
    - 'geometric':   alpha_t = (1 - t)^2

  Args:
    name: One of the schedule names listed above.

  Returns:
    A (alpha_fn, alpha_derivative_fn) tuple, where each function maps a
    scalar or array t to the corresponding value.

  Raises:
    ValueError: If `name` is not a recognised schedule.
  """
