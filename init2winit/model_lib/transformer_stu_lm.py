...
class TransformerSTUHybridBlock(nn.Module):
  ...
  def __call__(self,
    ...
    if self.do_stu_proj:
      project_fn = jax.vmap(compute_xtilde, in_axes=[0, None, None])
      projection = project_fn(x, self.eigh, conv_fft)
    else:
      projection = x
    logging.info('x shape 2 : %s', x.shape)
    init = (
        nn.initializers.zeros_init()
        if self.zero_init_input
        else nn.initializers.lecun_normal()
    )
    projection = nn.Dense(inputs.shape[-1], use_bias=False, kernel_init=init)(
        projection
    )
    logging.info('x shape 3 : %s', x.shape)
...
class TransformerLM(nn.Module):
  ...
  def setup(self):
    ...
    self.eigh = (
        eig_vals[-self.num_eigh:],
        eig_vecs[:, -self.num_eigh:],
    )
...
class TransformerLM1B(base_model.BaseModel):
  ...
  def build_flax_module(self):
    ...
    return TransformerLM(
        ...
        do_stu_proj=self.hps.do_stu_proj,
