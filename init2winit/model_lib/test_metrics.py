...
class MetricsTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          weights=np.array([[1.0, 1.0], [0.0, 1.0], [1.0, 0.0]]),
          result=0.5,
      ),
  )
  ...
  def test_wer_spm(self):
    ...
    with mock.patch.object(
      ...
      with mock.patch.object(
          tokenizer, 'detokenize', return_value=[source_sentence], autospec=True
      ):
        source_tokens = jnp.array(
            [tokenizer.tokenize(source_sentence)], dtype=jnp.int32
        )
        source_paddings = jnp.array([[0.0, 0.0, 0.0]], dtype=jnp.float32)

        decoded_tokens = jnp.array(
            [tokenizer.tokenize(decoded_sentence)], dtype=jnp.int32
        )
