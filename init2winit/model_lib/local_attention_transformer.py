...
import itertools
import math
from typing import Any, Union, Tuple, List, Dict, Sequence
...
DEFAULT_HPARAMS = config_dict.ConfigDict(
    dict(
        ...
        padding_bias=None,
        decoder_dropout_a=0.9,
        total_key_depth=None,
        total_value_depth=None,
        bias_cache=None,
        local_num_heads=8,
        cache=None,
...
class FeedForward(nn.Module):
  ...
  def __call__(self, input_x: Tensor, train: bool = False) -> Tensor:
    ...
    Args:
      input_x: an array with the decoder output.
...
def shape_list(input_array: Tensor) -> List[int]:
  """Outputs an array shape as a list."""
  output = list(map(int, list(input_array.shape)))
  return output
...
def get_item_at_decode_step(
  ...
  Args:
    input_array: an array with a single item to extract.
    decode_step: an integer decode step.
    query_shape: a tuple with a query shape.

  Returns:
    output: an array that is a single element from `x` at
    `decode_step` w.r.t. blocked raster scan order.
  """
  x_shape = shape_list(input_array=input_array)
  index = decode_step_to_index(
      decode_step=decode_step,
      array_shape=x_shape[1:-1],
      query_shape=query_shape)
  index = [i.tolist() for i in index]
  output = input_array[0:x_shape[0], index[0]:index[0] + len(index),
                       0:x_shape[-1]]
  return output
...
def embedding_to_padding(embedding: Tensor) -> Tensor:
  ...
  Args:
    embedding: an array with embeddings.

  Returns:
    output: an array where each element is 1 if its corresponding
...
def causal_attention_bias_nd(
  ...
  Returns:
    output: an array with a masked attention bias.
  """
...
def maybe_tile(input_x: Tensor, input_y: Tensor) -> Tensor:
  ...
  Args:
    input_x: a tuple with a query shape.
    input_y: a tuple with a memory flange shape.

  Returns:
    output: two arrays with the same shape except for batch and depth.
  """
  x_shape = shape_list(input_array=input_x)
  y_shape = shape_list(input_array=input_y)
...
def local_attention_bias_nd(
  ...
  Args:
    v_array: a [batch, num_blocks, items_in_blocks, depth] tensor for v.
    query_shape: a tuple with a query shape.
    memory_flange: a tuple with a memory flange shape.
  ...
  Returns:
    output: the local attention bias array.
  """
...
def pad_to_multiple_nd(input_x: Tensor, block_shape: Tuple[int]) -> Tensor:
  ...
  Returns:
    ...
    block length.
  """
  shape = shape_list(input_array=input_x)
...
def select_block_for_decode_step(
  ...
  Args:
    input_x: an array with a block to extract.
    decode_step: an integer decode step.
    query_shape: a tuple with a query shape.

  Returns:
     output: a array with the extracted block.
  """
  blocked_x_shape = shape_list(input_array=input_x)
  ...
  output = input_x[0:blocked_x_shape[0],
...
def break_into_blocks_nd(input_x: Tensor, block_shape: Tuple[int]) -> Tensor:
  ...
  Args:
    input_x: an array to be broken down into blocks.
    block_shape: a block shape.

  Returns:
    output: an input array broken down into blocks.
  """
  x_shape = shape_list(input_array=input_x)
...
def break_into_memory_blocks_nd(
  ...
  Returns:
    output: an input array split into memory blocks.
  """
  if all([m % q != 0 for q, m in zip(query_shape, memory_flange)]):
    raise ValueError(f'Query ({query_shape}) and memory ({memory_flange})'
                     ' modulo not equal to 0.')
  original_x_shape = shape_list(input_array=input_x)
...
def break_bias_into_blocks(
  ...
  x = jnp.reshape(x, [-1] + shape_list(input_array=x)[2:])
...
def generate_relative_positions_matrix(
  ...
  Returns:
    output: a matrix of relative positions.
  """
  if decode_step is None:
...
class RelativePositionEmbeddings(nn.Module):
  ...
  Attributes:
    ...
    query_shape: a tuple with a query shape.
    embeddings_init: embeddings initializer.
  """
  max_relative_position: int = 513
  query_shape: Tuple[int] = (256,)
  decode_step: int = None
  embeddings_init: Any = nn.initializers.glorot_uniform()

  @nn.compact
  def __call__(self, length_q: int, length_k: int, depth: int) -> Tensor:
    """Applies the RelativePositionEmbeddings module.

    Args:
      length_q: length of the query array.
      length_k: length of the key array.
      depth: depth of the output array.

    Returns:
      output: an array of relative positions of specified shape.
    """
    ...
    embeddings_array = self.param('embeddings_array', self.embeddings_init,
                                  (vocab_size, depth))
    output = jnp.take(
        embeddings_array, indices=relative_positions_matrix, axis=0)
    return output
...
def relative_attention_inner(input_x: Tensor,
  ...
  Args:
    input_x: first input array.
    input_y: second input array.
    input_z: third input array.
    transpose: whther to transpose inner matrices of input_y and input_z.

  Returns:
    output: an array with calculated relative dot-product attention.
  """
  if transpose:
    xy_matmul = jnp.einsum('bhxd,bhyd->bhxy', input_x, input_y)
    x_tz_matmul_r_t = jnp.einsum('bhxd,xyd->bhxy', input_x, input_z)
  else:
    xy_matmul = jnp.einsum('bhxd,bhdy->bhxy', input_x, input_y)
    x_tz_matmul_r_t = jnp.einsum('bhxd,xdy->bhxy', input_x, input_z)
...
class RelativeDotProductAttention(nn.Module):
  ...
  def __call__(
    ...
    Args:
      q_array: a query array.
      k_array: a key array.
      v_array: a value array.
      bias_array: a bias array.
    ...
    length_k = shape_list(input_array=k_array)[2]
    length_q = shape_list(input_array=q_array)[2]

    relations_keys = RelativePositionEmbeddings(
        max_relative_position=self.max_relative_position,
        query_shape=self.query_shape,
        decode_step=self.decode_step)(
            length_q=length_q, length_k=length_k, depth=depth)
...
def combine_heads_nd(input_x: Tensor) -> Tensor:
  ...
  Args:
    input_x: an array to be processed.

  Returns:
    output: a processed array.
  """
  num_dimensions = len(shape_list(input_array=input_x)) - 3
...
def put_back_blocks_nd(input_x: Tensor, block_shape: Tuple[int]) -> Tensor:
  ...
  Args:
    input_x: an input array for restructuring.
    block_shape: a block shape.

  Returns:
    output: an output array with normal ordering.
  """
  x_shape = shape_list(input_array=input_x)
...
class LocalAttention(nn.Module):
  ...
  def __call__(self,
    ...
    Returns:
      output: an array with local attention, an array with attention
      weights
    """
    ...
    if self.decode_step is not None:
      q_array = jnp.reshape(
          q_array, [-1] + shape_list(input_array=q_array)[2:])
      ...
      q_array = jnp.reshape(
          ...
          shape_list(input_array=q_array)[1:])
      latest_q = jnp.reshape(
          latest_q,
          [-1, self.local_num_heads] +
          shape_list(input_array=latest_q)[1:])
      q_shape = shape_list(input_array=latest_q)
    else:
      q_shape = shape_list(input_array=q_array)
    k_shape = shape_list(input_array=k_array)
    v_shape = shape_list(input_array=v_array)
    ...
    v_array = break_into_memory_blocks_nd(
        ...
        memory_flange=self.memory_flange,
        masked=self.masked)
    blocks_per_dim = shape_list(input_array=q_array)[1:-2]
    ...
    output = jnp.reshape(
        ...
        shape_list(input_array=output)[1:])
    outputs.append(output)

    output = jnp.concatenate(outputs, axis=1)
    output_shape = shape_list(input_array=output)
    output = jnp.reshape(
        output,
        [output_shape[0], self.local_num_heads, -1, output_shape[-1]])
    output = nn.Dense(
        output_shape[-1], kernel_init=self.kernel_init,
        use_bias=False)(
            output)
    output = jnp.reshape(output, output_shape)
    ...
    if self.decode_step is None:
      output = jnp.reshape(
          output, [-1] + shape_list(input_array=output)[2:])
      output = put_back_blocks_nd(
          input_x=output, block_shape=self.query_shape)
      output = jnp.reshape(
          output, q_shape[:2] + shape_list(input_array=output)[1:])
      output_shape = shape_list(input_array=output)
...
def split_heads_nd(input_x: Tensor, num_heads: int = 8) -> Tensor:
  ...
  Args:
    input_x: an input array to split into multiple heads.
    num_heads: a number of attention heads.

  Returns:
    output: an array split into multiple heads.
  """
  num_dimensions = len(shape_list(input_array=input_x)) - 2
  x_shape = shape_list(input_array=input_x)
  m = x_shape[-1]
...
def put_item_in_decode_step(
  ...
  Args:
    input_x: an array where the specified values will be replaced
    decode_step: the decode step in fast decoding mode.
    query_shape: a tuple with a query shape.
    replacement: new values that will be used as a replacement.

  Returns:
    output: an array with value at `decode_step`
...
class MultiHeadAttention(nn.Module):
  ...
  total_key_depth: int = 1024
  total_value_depth: int = 1024
  query_shape: Tuple[int] = (256,)
  memory_flange: Tuple[int] = (256,)
  local_num_heads: int = 8
  local_relative: bool = True
  ...
  def __call__(self, query_antecedent: Tensor, train: bool = False) -> Tensor:
    ...
    Args:
      query_antecedent: a query antecedent array.
      train: whether a deterministic mode or not.

    Returns:
      output: an array with calculated multi-head dot product attention.
    """
    ...
    if self.share_qk:
      if self.memory_antecedent:
        raise ValueError(f'Memory ({self.mmemory_antecedent}) is not None')
    ...
    if self.decode_step is not None:
      latest_antecedent = get_item_at_decode_step(
          ...
          query_shape=self.query_shape)
      latest_q = nn.Dense(
          self.total_key_depth, kernel_init=self.kernel_init, use_bias=False)(
              latest_antecedent)
      latest_k = nn.Dense(
          self.total_key_depth, kernel_init=self.kernel_init, use_bias=False)(
              latest_antecedent)
      latest_v = nn.Dense(
          self.total_value_depth, kernel_init=self.kernel_init, use_bias=False)(
              latest_antecedent)
    ...
    output = nn.Dense(
        self.hidden_size, kernel_init=self.kernel_init, use_bias=False)(
            output)
    return output
...
def get_timing_signal_1d(length: int,
  ...
  Returns:
    outputs: an array with timing signal.
  """
...
class ProcessInput(nn.Module):
  ...
  def __call__(self, input_x: Tensor, train: bool = False) -> Tensor:
    ...
    Args:
      input_x: a tokenized input array to preprocess.
      train: whether a deterministic mode or not.

    Returns:
      output: a preprocessed input sequence.
    """
    shape = shape_list(input_array=input_x)
    if len(shape) != 2:
    ...
    output = jnp.take(embeddings_array, output, axis=0)
    ...
    output = nn.Dense(
        self.hidden_size, kernel_init=self.kernel_init, use_bias=True)(
            output)
...
def process_partial_targets_decoding(
  ...
  Args:
    targets: length of timing signal sequence.
    query_shape: a tuple with a query shape.

  Returns:
    outputs: a processed input array.
  """
  targets_shape = shape_list(input_array=targets)
...
class LayerPostProcess(nn.Module):
  ...
  post_attention_dropout: float = 0.9
  ...
  def __call__(self,
    ...
    Args:
      input_x: an array with the decoder output.
      input_y: an array with the decoder and FeedForward layer output.
      train: whether a deterministic mode or not.

    Returns:
      output: a postprocessed array.
    """
    x = nn.Dropout(
        rate=self.post_attention_dropout, deterministic=not train)(input_x)

    x = x + input_y

    output = nn.LayerNorm(
        epsilon=self.post_attention_epsilon)(x)
...
class DecoderBlock(nn.Module):
  ...
  bias_cache: Dict[str, Tensor] = None
  memory_antecedent: Tensor = None
  total_key_depth: int = None
  total_value_depth: int = None
  query_shape: Tuple[int] = (256,)
  memory_query_shape: Tuple[int] = (512,)
  ...
  def __call__(self, input_x: Tensor, train: bool = False) -> Tensor:
    ...
    Args:
      input_x: a preprocessed tokenized input sequence.
      train: boolean indicating whether training or not.

    Returns:
      output: an array modified by a decoder module.
    """
    x = nn.Dropout(
    ...
    y = MultiHeadAttention(
        ...
        share_qk=self.share_qk,
        token_bias=self.token_bias,
        padding_bias=self.padding_bias)(query_antecedent=x, train=train)
...
class LocalAttentionTransformerArchitecture(nn.Module):
  ...
  def __call__(self,
    ...
    x = ProcessInput(
        ...
        max_target_length=self.max_target_length)(input_x=x, train=train)
    ...
    for layer in range(self.num_decoder_layers):
      y = DecoderBlock(
          ...
          post_attention_dropout=self.post_attention_dropout,
          feedforward_depths=self.feedforward_depths,
          feedforward_dropout=self.feedforward_dropout)(input_x=x, train=train)
